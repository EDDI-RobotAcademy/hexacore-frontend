...
export default function HealthIndicator({ data, loading, error }: Props) {
  ...
  const redisStatus = getStatusColor(data?.redis);
          ...
          <div className="p-3 space-y-2">
            ...
            <div className="flex items-center justify-between px-3 py-2 bg-white/[0.03] rounded-lg">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm text-white/70">Redis 캐시</span>
              </div>
              <div className="flex items-center gap-2">
                <StatusDot status={loading ? "unknown" : redisStatus} />
                <span className={`text-xs font-medium ${redisStatus === "ok" ? "text-emerald-400" : redisStatus === "error" ? "text-rose-400" : "text-amber-400"}`}>
                  {loading ? "..." : data?.redis || "-"}
                </span>
              </div>
            </div>
